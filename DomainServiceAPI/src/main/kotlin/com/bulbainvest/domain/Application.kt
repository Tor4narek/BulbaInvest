...
fun Application.module() {
    ...
    configureSecurity(cfg.jwt, get<JwtService>())

    routing {
        get("/health") { call.respond(mapOf("status" to "ok")) }

        authRoutes(get<AuthCodeStore>(), get<MailService>(), get<UserService>(), get<JwtService>())
        userRoutes(get<UserService>())
        walletRoutes(get<WalletService>())
        portfolioRoutes(get<PortfolioService>())
        tradeRoutes(get<TradeService>())
        orderRoutes(get<OrderService>())
    }
}
